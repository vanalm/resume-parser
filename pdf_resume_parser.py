...
load_dotenv()
csv_file_name = 'missing_resume_data3.csv'
directory_path =  "./missing_resumes"
...
# Function to extract fields from JSON data
def extract_fields_from_json(data, resume_filename):
    # Safely access ContactInformation
    contact_info = data['Value']['ResumeData'].get('ContactInformation', {})
    education_info = data['Value']['ResumeData'].get('Education', {})
    employment_info = data['Value']['ResumeData'].get('EmploymentHistory', {})
    skills_info = data['Value']['ResumeData'].get('Skills', {}).get('Raw', [])
    ...
    fields = {
        'Name in special format': f"{contact_info.get('CandidateName', {}).get('FamilyName', '')}-{contact_info.get('CandidateName', {}).get('GivenName', '')}-{contact_info.get('CandidateName', {}).get('MiddleName', '')}",
        'last name': contact_info.get('CandidateName', {}).get('FamilyName', ''),
        'first name': contact_info.get('CandidateName', {}).get('GivenName', ''),
        'email address': contact_info.get('EmailAddresses', [None])[0],
...
print(f"All results have been concatenated and saved to the current directory as {csv_file_name}")
